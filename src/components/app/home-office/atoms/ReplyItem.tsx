...
const ReplyItem = ({ reply, editable, deletable, onDelete }: Props) => {
    const [likes, setLikes] = useSafeState(reply.reactionCount || 0);
    ...
    const onEdit = async (payload: TCommentSchema) => {
        ...
        onEditComment({
            ...
            method: 'PUT',
            url: `${reply.homeOfficeId}/comment`,
            payload: payload,
        });
    };
    ...
    return (
        <div className="flex flex-col gap-3 mt-3">
            <div className="ml-3 flex-col gap-1" id="No reply items">
                <div className="flex gap-4 justify-between">
                    <div className="flex gap-4">
                        <EducareImage
                            variant="avatar"
                            preview={false}
                            image={{ imagePath: reply.userInfo.profilePic, blurHash: reply.userInfo.profilePicBlurHash }}
                        />
                        <div className="mb-2">
                            <div className="flex justify-between">
                                <div className="text-lg font-semibold">{reply.userInfo.name || ''}</div>
                            </div>
